...
- A user-authorized live microphone start connected successfully, but the opening Realtime response incorrectly saved the synthetic example name `John Smith` before the user spoke. This trust-blocking defect was reproduced in the review UI and fixed in commit `aa0a2f8` with a deterministic start→stop user-turn guard, one-write-per-turn consumption, reconnect reset, and stricter ask-then-wait instructions. The full 79-test and evaluation suite passed, Render deployed the commit successfully, `/api/health` returned HTTP 200, and the public client bundle was confirmed to contain the `user_turn_required` guard. A final human audio retest is still required before claiming the voice path passed live.

The live `429` ceiling was not exhausted because that would deliberately spend the public visitor/address budgets and additional model requests; the deterministic limiter suite covers the 3-per-visitor compile and 10-per-visitor verification/Realtime rules. The post-fix microphone behavior and live rate-limit ceiling remain explicit manual/operational checks rather than claimed live passes.
