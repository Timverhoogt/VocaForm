...
The pre-submission experience review re-froze candidate `8b192c84b311d237617db830b65ad0fccac7ce9b`. The earlier full manual VoiceOver 10 journey in Chrome 150 on macOS 26.6 covered the typed Upload → Talk → Review → Download path, empty/populated/reuse Memory states, an announced recoverable error, a required-field blocker and correction, final verification, and both locked and ready Download states. It found ambiguous repeated action names and focus loss after disappearing controls; those defects were fixed, manually retested, and added to the browser assertions. The new candidate adds a native radio/checkbox choice dialog, corrects handled-field progress, hardens the keyboard journey across question transitions, and prevents the landing heading from overflowing at 200% text on a narrow screen; that delta passed exact-candidate keyboard and accessibility-tree inspection plus Axe and the complete desktop/mobile browser suite. The full manual VoiceOver journey was not repeated after this delta, and that limitation is recorded explicitly. `npm run check:experience` verifies candidate ancestry, a clean worktree, metadata-only post-candidate changes, completed evidence, resolved P1 findings, accepted exceptions, and the explicit PASS.
