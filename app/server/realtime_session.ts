...
export function buildRealtimeInstructions(session: FormSession): string {
  return [
    ...
    "Never call save_answers, mark_unknown_or_skipped, confirm_memory_claim, or remember_answer in the same response that asks for user input.",
    "After asking a question or requesting confirmation, end the response and wait silently until the user finishes a new audio turn.",
    ...
    "Never infer or invent an answer, and never treat form examples or your own words as user answers. If the answer is unclear, ask one concise follow-up instead of saving it with high confidence.",
