...
`npm run eval:compiler` evaluates the reviewed compiler outputs against 52 expected fields, 25 explicitly required fields, and the two conditional dependencies. It fails below 95% aggregate recall, below 100% required-field recall, on any fabricated field ID, or on a missing dependency.
