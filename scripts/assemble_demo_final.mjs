...
const root = path.resolve("work/video/iterations/v5-final");
...
const outputStem = "vocaform-final-updated-demo";
const realtimeMetadata = JSON.parse(await readFile(
  path.join(productDirectory, "scene-03-realtime-live.json"),
  "utf8"
));

const scene02Path = path.join(sceneDirectory, "scene-02-final.mp4");
const scene02Captions = path.join(root, "scene-02-final-corrected.srt");
const scene03Path = path.join(sceneDirectory, "scene-03-final-realtime.mp4");
const scene03Captions = path.join(root, "scene-03-final-realtime.srt");
const scene06Path = path.join(sceneDirectory, "scene-06-webform-final.mp4");
const scene07Path = path.join(sceneDirectory, "scene-07-evidence-final.mp4");
const scene08Path = path.join(sceneDirectory, "scene-08-close-final.mp4");

await mkdir(sceneDirectory, { recursive: true });
buildScene02();
await writeDocumentCaptions();
buildRealtimeScene();
await writeRealtimeCaptions();
buildWebformScene();
buildEvidenceScene();
buildCloseScene();
...
const scenes = [
  ...
  },
  {
    id: "evidence",
    file: scene07Path,
    captions: path.join(heygenDirectory, "scene-07-evidence-final.srt")
  },
  {
    id: "close",
    file: scene08Path,
...
await writeFile(path.join(root, "timeline.json"), `${JSON.stringify({
  ...
  editorial: {
    ...
    evidenceAvatarMotionRetimedWithoutFrozenPadding: true,
  ...
  heygen: {
    ...
    changedScenesOnly: ["document", "webform", "evidence", "close"],
...
function buildEvidenceScene() {
  ...
  runFfmpeg([
    ...
    "-i", path.join(heygenDirectory, "scene-07-evidence-final-avatar.mp4"),
    "-filter_complex",
    [
      "[0:v]fps=25,scale=1920:1080[bg]",
      `[1:v]setpts=1.04806*PTS,crop=2160:2160:840:0,scale=416:416,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='${circleMask}'[avatar]`,
...
function correctEditorialCaptions(id, cues) {
  ...
  if (id === "evidence" && cues.length >= 8) {
    cues[3].lines = ["gates, and 122"];
    cues[6].lines = ["53 expected fields with zero"];
  }
  if (id === "close") {
